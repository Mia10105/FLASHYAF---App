    ...
    >
      ...
      >
        ...
        <g style={{ animation: "flameDance1 2.1s ease-in-out infinite" }}>
          <path
            d="M165,300 C140,255 118,225 128,185 C136,150 148,125 152,108 C156,88 175,68 205,55 C232,70 248,92 250,115 C253,145 262,175 248,215 C238,248 232,278 235,300 C212,308 188,308 165,300Z"
            fill="url(#fg1)"
          />
        </g>
        <g style={{ animation: "flameDance2 1.7s ease-in-out infinite" }}>
          <path
            d="M185,300 C165,265 150,240 158,205 C164,178 172,158 178,145 C183,130 195,118 208,110 C222,120 230,135 232,152 C235,175 228,195 222,225 C217,255 213,280 215,300 C205,306 195,306 185,300Z"
            fill="url(#fg2)"
          />
        </g>
        <g style={{ animation: "flameDance3 2.4s ease-in-out infinite" }}>
          <path
            d="M112,300 C95,265 82,235 90,205 C96,182 104,165 112,155 C120,145 130,138 138,135 C146,145 150,158 148,172 C146,190 138,205 132,230 C127,255 122,280 128,300 C123,304 117,304 112,300Z"
            fill="url(#fg1)"
            opacity="0.6"
          />
        </g>
        ...
        >
          <path
            d="M255,300 C240,262 230,230 238,198 C244,175 252,158 262,148 C270,140 280,136 288,140 C294,150 296,164 292,180 C288,200 280,218 276,245 C272,268 268,285 272,300 C266,304 260,304 255,300Z"
            fill="url(#fg1)"
            opacity="0.6"
          />
        </g>
...
export default function LiveFlashScreen({ onComplete }: Props) {
  ...
  const [sessionOwnerUid, setSessionOwnerUid] = useState<string | null>(
    savedFlashState?.ownerUid ?? user?.uid ?? null,
  );
  const sessionOwnerUidRef = useRef<string | null>(sessionOwnerUid);

  useEffect(() => {
    if (!sessionOwnerUid && user?.uid) {
      sessionOwnerUidRef.current = user.uid;
      setSessionOwnerUid(user.uid);
    }
  }, [sessionOwnerUid, user]);

  useEffect(() => {
    sessionOwnerUidRef.current = sessionOwnerUid;
  }, [sessionOwnerUid]);
  ...
  }, [currentStageIdx, stages, bodyAreas, sessionOwnerUid]);
  useEffect(() => {
  ...
  handleCompleteRef.current = async (completedStages: StageEntry[]) => {
    ...
    const { notes: _localNotes, id: _localId, ...flashWithoutLocalNotes } = flash;
    const flashForCloud = journalPref === "local" ? flashWithoutLocalNotes : (() => {
      const { id: _id, ...cloudFlash } = flash;
      return cloudFlash;
    })();

    // Local-only notes are saved under the same pre-generated document ID
    // that a later retry will use. This keeps the local content attached to
    // the correct flash without ever putting the note text in the cloud queue.
    if (journalPref === "local" && allNotes) {
      await saveLocalFlashContent(documentId, { notes: allNotes });
    ...
    if (ownerUid && user?.uid === ownerUid) {
      try {
        await setDoc(
          doc(db, "users", ownerUid, "flashes", documentId),
          flashForCloud,
        );
      } catch {
        try {
          ...
          pending.push({
            flashData: flashForCloud,
            userId: ownerUid,
            documentId,
            localOnly: journalPref === "local",
          });
      ...
      // Authentication disappeared or changed while the flash was active.
      // Queue only for the immutable account that owned the session; never
      // let another account on this device claim it.
      try {
        const PENDING_KEY = "flashyaf_pending_flashes";
        const pending = JSON.parse(localStorage.getItem(PENDING_KEY) || "[]");
        pending.push({
  ...
  useEffect(() => {
    ...
    void (async () => {
      const remaining: PendingFlash[] = [];
      for (const item of pending) {
        // Never auto-claim legacy records that have no immutable owner.
        if (!item.userId || item.userId !== user.uid) {
          remaining.push(item);
          continue;
        }

        const documentId = item.documentId || item.flashData.id;
        if (!documentId) {
          remaining.push(item);
          continue;
        }

        const { notes: _notes, id: _id, ...withoutNotes } = item.flashData;
        const { id: _cloudId, ...withPossibleNotes } = item.flashData;
        // New queue entries explicitly record the capture-time preference.
        // Legacy entries default to local-only so a later preference change
        // can never cause old note text to be uploaded unexpectedly.
        const toUpload = item.localOnly === false ? withPossibleNotes : withoutNotes;
        toUpload.userId = user.uid;

        try {
          await setDoc(
            doc(db, "users", user.uid, "flashes", documentId),
            toUpload,
          );
        } catch {
          remaining.push(item);
