...
export default function SettingsScreen({ onNavigate }: Props) {
    ...
    async function loadPartner() {
      try {
        ...
        if (snap.exists()) {
          ...
          setResearchConsent(!!data.researchConsent);
  ...
  async function handleToggleResearchConsent() {
    if (!user) return;
    ...
    try {
      ...
      // Revert the optimistic UI update if the save failed.
      setResearchConsent(!next);
    }
  }
  ...
  async function handleDeleteData(mode: "immediate" | "pause") {
    ...
    try {
      ...
      await requestAccountDeletion({ mode });

      // Cloud Functions can delete server data, but browser storage must be
      // cleared while the app is open. This happens for both choices because
      // a scheduled day-91 server deletion cannot reach this device later.
      try {
        localStorage.clear();
      ...
      try {
        await new Promise<void>((resolve) => {
          const request = indexedDB.deleteDatabase("flashyaf_local_content");
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.content}>
        ...
        <div style={{ ...styles.section, border: researchConsent ? "1px solid rgba(155,89,182,0.35)" : "1px solid rgba(255,255,255,0.08)", background: researchConsent ? "rgba(155,89,182,0.06)" : "var(--color-card)" }}>
          <p style={styles.sectionLabel}>Research Participation</p>
          <div style={styles.partnerToggleRow}>
            <div style={styles.partnerToggleLeft}>
              <span style={styles.partnerIcon}>🔬</span>
              <div>
                <p style={styles.partnerToggleTitle}>Share anonymous data for research</p>
                <p style={styles.partnerToggleSub}>Helps improve FLASHYAF™ for everyone. Never your name, email, or anything identifying — always revocable here, anytime, with no effect on your account or pricing.</p>
              </div>
            </div>
            <div style={{ ...styles.toggle, background: researchConsent ? "#9B59B6" : "rgba(255,255,255,0.15)" }} onClick={handleToggleResearchConsent} role="switch" aria-checked={researchConsent}>
              <div style={{ ...styles.toggleKnob, left: researchConsent ? "22px" : "3px" }} />
            </div>
          </div>
        </div>
          ...
          <div style={styles.modalSheet}>
            {deleting ? (
              <div style={styles.deletingState}>
                ...
                <p style={styles.deletingTitle}>Processing your request…</p>
                <p style={styles.deletingSub}>Please keep the app open until this finishes.</p>
              </div>
            ) : (
              <>
                <div style={styles.modalIconWrap}><span style={styles.modalIcon}>⚠️</span></div>
                <p style={styles.modalTitle}>Delete My Account</p>
                <p style={styles.modalSub}>Choose how you'd like to proceed.</p>
                ...
                <button style={{ ...styles.modalDeleteBtn, background: deleteInput.trim().toUpperCase() === "DELETE" ? "linear-gradient(135deg, #922B21 0%, #C0392B 100%)" : "rgba(255,255,255,0.06)", color: deleteInput.trim().toUpperCase() === "DELETE" ? "#fff" : "rgba(255,255,255,0.2)", cursor: deleteInput.trim().toUpperCase() === "DELETE" ? "pointer" : "default" }} onClick={() => handleDeleteData("immediate")} disabled={deleteInput.trim().toUpperCase() !== "DELETE"}>
                  🗑️ Delete Now — Permanently
                </button>
                <button style={{ ...styles.modalDeleteBtn, background: deleteInput.trim().toUpperCase() === "DELETE" ? "linear-gradient(135deg, #6C3483 0%, #8E44AD 100%)" : "rgba(255,255,255,0.06)", color: deleteInput.trim().toUpperCase() === "DELETE" ? "#fff" : "rgba(255,255,255,0.2)", cursor: deleteInput.trim().toUpperCase() === "DELETE" ? "pointer" : "default", marginTop: "10px" }} onClick={() => handleDeleteData("pause")} disabled={deleteInput.trim().toUpperCase() !== "DELETE"}>
                  ⏸️ Pause for 90 Days Instead
                </button>
                <p style={{ ...styles.modalSub, fontSize: "12px", marginTop: "8px" }}>Pausing preserves your cloud account data for 90 days so you can change your mind by logging back in. Local-only notes and recordings on this device are removed when the pause begins and cannot be restored. If you don't return, the remaining cloud data is permanently deleted automatically.</p>
