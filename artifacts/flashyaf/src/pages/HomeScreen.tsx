...
import {
  type JournalEntry,
  loadJournalEntries as loadJournalEntriesFromStorage,
  deleteJournalEntry as deleteJournalEntryFromStorage,
  getJournalPreference,
...
const TIPS: { category: string; tip: string }[] = [
  {
    category: "Hydration",
    tip: "Keep a chilled water bottle within arm's reach. Sipping cold water at the first sign of a flash can cut its intensity — your body is listening.",
  },
  {
    category: "Hydration",
    tip: "Try cucumber-infused water. It's cooling, hydrating, and honestly feels a little luxurious. You deserve luxurious.",
  },
  {
    category: "Hydration",
    tip: "Aim for at least 8 glasses of water today. Every cell in your body is doing overtime right now — hydration is your most powerful free tool.",
  },
  {
    category: "Cooling",
    tip: "Keep a small personal fan on your nightstand and desk. Immediate airflow is the fastest flash de-escalator you have.",
  },
  {
    category: "Cooling",
    tip: "Layer your clothing. Loose, breathable layers you can peel off quickly are your secret weapon. Natural fabrics like cotton and linen are your friends.",
  },
  {
    category: "Cooling",
    tip: "Cold water on your wrists and the back of your neck cools your whole body fast — pulse points are your shortcut to relief.",
  },
  {
    category: "Breathing",
    tip: "Try paced breathing: inhale for 4 counts, hold for 4, exhale for 6. Studies show slow breathing can reduce flash frequency by up to 50%.",
  },
  {
    category: "Breathing",
    tip: "When a flash hits, slow everything down. Your breath is the one thing you can always control — and it controls your body temperature more than you think.",
  },
  {
    category: "Breathing",
    tip: "Box breathing (4-4-4-4) activates your parasympathetic nervous system. It tells your brain to stop panicking. Practice it before you need it.",
  },
  {
    category: "Sleep",
    tip: "Keep your bedroom between 60–67°F. A cooler sleep environment is one of the most evidence-backed ways to reduce night sweats.",
  },
  {
    category: "Sleep",
    tip: "Consider a cooling mattress topper or moisture-wicking sheets. Your bedding shouldn't fight you — it should be on your side.",
  },
  {
    category: "Sleep",
    tip: "Avoid alcohol within 3 hours of bedtime. It fragments your sleep and can trigger night sweats even hours after your last sip.",
  },
  {
    category: "Nutrition",
    tip: "Spicy foods, caffeine, and alcohol are the most common flash triggers. Try cutting one for a week and see if things calm down. Data > guesswork.",
  },
  {
    category: "Nutrition",
    tip: "Soy contains phytoestrogens that may help reduce flash frequency for some women. Edamame, tofu, and soy milk are easy additions.",
  },
  {
    category: "Nutrition",
    tip: "Flaxseeds are a natural source of lignans — plant compounds with mild estrogen-like effects. Add a tablespoon to your morning smoothie.",
  },
  {
    category: "Exercise",
    tip: "Regular cardio exercise can reduce hot flash frequency by up to 55%. Even a 20-minute brisk walk counts. Every step is working for you.",
  },
  {
    category: "Exercise",
    tip: "Yoga has strong evidence for reducing menopause symptoms — not just flexibility. The breathing, the movement, the stillness. It all helps.",
  },
  {
    category: "Exercise",
    tip: "Strength training preserves bone density, boosts mood, and helps regulate body temperature. Lifting things is literally medicine right now.",
  },
  {
    category: "Stress",
    tip: "Stress is one of the most overlooked flash triggers. Even 5 minutes of mindfulness per day can lower your cortisol levels meaningfully.",
  },
  {
    category: "Stress",
    tip: "Identify your personal flash triggers with your tracking data. Patterns will emerge — and patterns give you control back.",
  },
  {
    category: "Stress",
    tip: "Talk to someone you trust about what you're going through. Isolation amplifies every symptom. Connection is biological medicine.",
  },
];
...
const FEATURED_MERCH = [
  ...
  },
  {
    emoji: "🎁",
    name: "Gift Sets",
    tagline: "For the woman who needs this",
    color: "#B8860B",
...
function shouldShowRatingPrompt(flashCount: number): boolean {
  ...
  const lastDismissed = parseInt(
    localStorage.getItem(RATING_DISMISSED_KEY) || "0",
    10,
  );
...
function buildInsights(flashes: Flash[]): { icon: string; text: string }[] {
  ...
  flashes.forEach((f) => {
    byHour[new Date(f.startTime).getHours()]++;
  });
  const peakH = byHour.indexOf(Math.max(...byHour));
  insights.push({
    icon: "🕐",
    text: `Your flashes tend to peak ${hourRangeLabel(peakH)}. Knowing when to expect them gives you a real edge — you can prepare, not just react.`,
  });
  const avgSec =
    flashes.reduce((s, f) => s + f.durationSeconds, 0) / flashes.length;
  ...
  insights.push({
    icon: "⏱️",
    text: `Your average flash lasts ${avgMin} minutes — ${comparison} the typical 4-minute flash. Every body runs on its own schedule.`,
  });
  const firstTs = flashes[flashes.length - 1].startTime;
  const daysSince = Math.max(1, Math.round((Date.now() - firstTs) / 86400000));
  insights.push({
    icon: "📅",
    text: `You've been tracking for ${daysSince} day${daysSince !== 1 ? "s" : ""} — ${flashes.length} flash${flashes.length !== 1 ? "es" : ""} logged. That kind of consistency is rare.`,
  });
  ...
  if (ratedFlashes.length >= 3) {
    ...
    ratedFlashes.forEach((f) => {
      const d = new Date(f.startTime).getDay();
      byDay[d].sum += f.peakRating!;
      byDay[d].count++;
    });
    const peak = byDay
      .map((d, i) => ({ day: i, avg: d.count > 0 ? d.sum / d.count : 0 }))
      .reduce((a, b) => (a.avg >= b.avg ? a : b));
    insights.push({
      icon: "📊",
      text: `Your most intense flashes tend to happen on ${DAY_NAMES[peak.day]}. A lighter schedule on ${DAY_NAMES[peak.day]} could help.`,
    });
  } else {
    const byDayCount = Array(7).fill(0) as number[];
    flashes.forEach((f) => {
      byDayCount[new Date(f.startTime).getDay()]++;
    });
    const busiestDay = byDayCount.indexOf(Math.max(...byDayCount));
    insights.push({
      icon: "📊",
      text: `${DAY_NAMES[busiestDay]} tend to be your highest-flash days. A little extra self-care on ${DAY_NAMES[busiestDay]} goes a long way.`,
    });
  }
  const tod = timeOfDayLabel(peakH);
  insights.push({
    icon: "💡",
    text: `Pattern tip: Keep a cold drink and a small fan nearby in the ${tod} — that's when your body tends to need it most.`,
  });
  return insights;
}
...
export default function HomeScreen({
  ...
  const [capsuleMilestone, setCapsuleMilestone] = useState<30 | 60 | 90 | null>(
    null,
  );
  ...
  // FIX: entries can now be saved to the cloud (synced across devices) or
  // kept phone-only, based on the user's own choice — see journalStorage.ts
  const [showJournal, setShowJournal] = useState(false);
  const [showJournalStorageChoice, setShowJournalStorageChoice] =
    useState(false);
  ...
  const [journalSpeakingId, setJournalSpeakingId] = useState<string | null>(
    null,
  );
  ...
  async function refreshJournalEntries() {
    ...
    setJournalEntries(await loadJournalEntriesFromStorage(uid));
  }
  function openJournal() {
    // First time opening the journal — ask where notes should be saved.
    if (getJournalPreference() === null) {
      setShowJournalStorageChoice(true);
      return;
    }
  ...
  function chooseJournalStorage(pref: "cloud" | "local") {
    setJournalPreference(pref);
    setShowJournalStorageChoice(false);
    refreshJournalEntries();
    setJournalDeleteId(null);
    setShowJournal(true);
  }
  async function deleteJournalEntry(id: string) {
    const uid = user?.uid;
    if (uid) await deleteJournalEntryFromStorage(uid, id);
    setJournalEntries((prev) => prev.filter((e) => e.id !== id));
    setJournalDeleteId(null);
  }
  function speakJournalEntry(entry: JournalEntry) {
    if (!("speechSynthesis" in window)) return;
    const stLabel = (JOURNAL_STAGE_LABELS[entry.stage] || entry.stage).replace(
      /^[^\w]+/,
      "",
    );
  ...
  useEffect(() => {
    const count = parseInt(
      localStorage.getItem(`flashyaf_checkin_count_${getTodayKey()}`) || "0",
      10,
    );
  ...
  useEffect(() => {
    function computeStats(loaded: Flash[]) {
      ...
      const weekFlashes = loaded.filter(
        (f) => f.startTime >= startOfWeek.getTime(),
      );
      const avgSec =
        loaded.reduce((sum, f) => sum + f.durationSeconds, 0) / loaded.length;
      ...
      setStats({
        lastFlash: lastStr,
        todayCount: todayFlashes.length,
        weekCount: weekFlashes.length,
        avgDuration: `${avgMin}m ${avgRemSec}s`,
      });
    }
    if (isDemo) {
    ...
    async function loadStats() {
      ...
      const loaded: Flash[] = snap.docs.map(
        (d) => ({ id: d.id, ...d.data() }) as Flash,
      );
      setFlashes(loaded);
      computeStats(loaded);
        ...
        if (milestone) {
          ...
          if (!localStorage.getItem(capsuleKey)) {
            import("firebase/firestore").then(({ doc, getDoc }) => {
              getDoc(
                doc(db, "users", user!.uid, "timeCapsules", `day${milestone}`),
              )
                .then((snap) => {
                  if (!snap.exists()) {
      ...
      if (userSnap.exists()) {
        ...
        if (data.supportStyle)
          setSupportStyle(data.supportStyle as "warm" | "direct" | "mindful");
        if (typeof data.prefersHumor === "boolean")
          setPrefersHumor(data.prefersHumor);
      }
    }
    loadStats();
  ...
  function showFlashyToast() {
    const phrase =
      FLASHY_PHRASES[Math.floor(Math.random() * FLASHY_PHRASES.length)];
  ...
  return (
    <div style={styles.container}>
      {activeFlashBanner && (
        <div style={styles.activeBanner}>
          <span style={styles.bannerText}>
            🔥 You have a flash in progress — pick up where you left off?
          </span>
          <div style={styles.bannerActions}>
            {/* FIX: this used to say "Go end it" but actually discarded all
                progress and started a brand-new flash. Now it genuinely
                resumes exactly where you left off — stage, timer, and all. */}
            <button
              style={styles.bannerResumeBtn}
              onClick={() => {
                setActiveFlashBanner(false);
                onStartTracking();
              }}
            >
              Resume
            ...
            <button
              ...
              onClick={() => {
                ...
                localStorage.removeItem("flashyaf_active_flash_state");
                setActiveFlashBanner(false);
              }}
            >
              End it
      ...
      <div style={styles.header}>
        ...
        {wakeWindowActive && (
          ...
          >
            ...
            />
            <span
              style={{ color: "#FF7043", fontSize: "11px", fontWeight: 700 }}
            >
      ...
      <div style={styles.content}>
        ...
        {partnerMsg && !partnerMsgDismissed && (
          <div
            style={{
              background:
                "linear-gradient(135deg, rgba(233,30,140,0.12), rgba(142,68,173,0.10))",
              ...
              <button
                ...
                onClick={() => {
                  ...
                  import("firebase/firestore").then(({ doc, updateDoc }) => {
                    updateDoc(doc(db, "partnerMessages", partnerMsg.id), {
                      read: true,
                    }).catch(() => {});
                  });
                }}
        ...
        {capsuleMilestone && !capsuleBannerDismissed && !showCapsule && (
          <div
            style={{
              background:
                "linear-gradient(135deg, rgba(255,107,53,0.12), rgba(192,57,43,0.08))",
          ...
          >
            ...
            <button
              ...
              onClick={(e) => {
                e.stopPropagation();
                setCapsuleBannerDismissed(true);
              }}
            >
              ✕
            </button>
          ...
          style={
            wakeWindowActive
              ? {
                  borderRadius: "100px",
                  boxShadow:
                    "0 0 0 6px rgba(255,69,0,0.25), 0 0 0 12px rgba(255,69,0,0.12), 0 0 40px rgba(255,69,0,0.5)",
        ...
        >
          or say "Hey Flashy, I feel it coming"
        </p>
        ...
        <div style={styles.statsGrid}>
          <div style={styles.statBox}>
            <span style={styles.statLabel}>Last Flash</span>
            <span style={styles.statValue}>{stats.lastFlash}</span>
          </div>
          <div style={styles.statBox}>
            ...
            <span
              ...
              style={{
                ...
                animation: todayCountAnim ? "countPop 0.45s ease" : undefined,
              }}
            >
              {displayTodayCount}
            </span>
          ...
          <div style={styles.statBox}>
            ...
            <span
              ...
              style={{
                ...styles.statValueLarge,
                animation: weekCountAnim ? "countPop 0.45s ease" : undefined,
              }}
            >
              {displayWeekCount}
            </span>
          ...
          <div style={styles.statBox}>
            ...
            <span style={styles.statValue}>{stats.avgDuration}</span>
          </div>
        </div>
          ...
          style={
            {
              ...
              width: "100%",
              textAlign: "left",
              background: autoRunBreathing
                ? "rgba(0,188,212,0.08)"
                : "rgba(255,255,255,0.04)",
              border: autoRunBreathing
                    ...
                    style={{
                      ...
                      background:
                        i === insightIndex
                          ? "rgba(142,68,173,0.9)"
                          : "rgba(255,255,255,0.15)",
                      width: i === insightIndex ? "16px" : "6px",
                    }}
                  />
                ))}
              </div>
              <span style={styles.insightRotate}>
                Updates daily · {insightIndex + 1} of {insights.length}
              </span>
            </div>
          </div>
        )}
              ...
              >
                <div style={styles.merchCalloutLeft}>
                  ...
                  <div style={styles.merchCalloutText}>
                    <p
                      style={{ ...styles.merchCalloutName, color: merch.color }}
                    >
                      {merch.name}
                    </p>
        ...
        {user?.email === ADMIN_EMAIL && (
          <button style={styles.adminBtn} onClick={() => onNavigate("admin")}>
            🔬 Research Dashboard
          </button>
        )}
      </div>
      ...
      <div style={styles.bottomNav}>
        ...
        <button style={styles.navBtn} onClick={() => onNavigate("history")}>
          <span>📋</span>
        ...
        <button style={styles.navBtn} onClick={() => onNavigate("learn")}>
          <span>📚</span>
          <span style={styles.navLabel}>Learn</span>
        </button>
        ...
        <DailyCheckInModal
          ...
          onSaved={() => {
            ...
            localStorage.setItem(
              `flashyaf_checkin_count_${getTodayKey()}`,
              String(next),
            );
      ...
      {flashyToast !== "" && (
        <div
          style={{
            ...
            gap: "10px",
            boxShadow:
              "0 8px 32px rgba(0,0,0,0.55), 0 0 0 1px rgba(255,69,0,0.12)",
      ...
      {/* ── FIX: first-time choice for where journal notes are saved ────── */}
      {showJournalStorageChoice && (
        <div
          style={{
            position: "fixed",
            inset: 0,
              ...
              >
                ...
                >
                  ...
                  <div>
                    ...
                    >
                      {journalEntries.length} entr
                      {journalEntries.length === 1 ? "y" : "ies"} · your
                      personal log
                    </p>
                  </div>
                </div>
            ...
            >
              {journalEntries.length === 0 ? (
                <div style={{ textAlign: "center", padding: "40px 20px" }}>
                  ...
                  >
                    Say "Hey Flashy, take a note" during a flash.
                  </p>
                </div>
              ) : (
                journalEntries.map((entry) => {
                  const stLabel =
                    JOURNAL_STAGE_LABELS[entry.stage] || entry.stage;
                  const stColor =
                    JOURNAL_STAGE_COLORS[entry.stage] || "#FF4500";
        ...
        <div style={styles.ratingPromptOverlay}>
          <div style={styles.ratingPromptCard}>
            ...
            <p style={styles.ratingPromptTitle}>
              You've been with us through {flashes.length} flash
              {flashes.length !== 1 ? "es" : ""}.
            </p>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  // FIX: calmer, smaller tap option — voice ("Hey Flashy, I feel it coming")
  // is now the primary way to start tracking; this stays as a quiet fallback.
  trackBtn: {
    background: "rgba(255,255,255,0.08)",
  ...
  insightCard: {
    ...
    boxSizing: "border-box",
    background:
      "linear-gradient(135deg, rgba(142,68,173,0.14) 0%, rgba(52,73,94,0.12) 100%)",
    border: "1.5px solid rgba(142,68,173,0.45)",
    borderRadius: "20px",
    padding: "16px 18px 14px",
    boxShadow:
      "0 0 28px rgba(142,68,173,0.1), inset 0 1px 0 rgba(142,68,173,0.18)",
  },
  insightHeader: {
    display: "flex",
    alignItems: "center",
  ...
  tipCard: {
    ...
    boxSizing: "border-box",
    background:
      "linear-gradient(135deg, rgba(245,166,35,0.07) 0%, rgba(245,166,35,0.03) 100%)",
    border: "1.5px solid rgba(245,166,35,0.55)",
    borderRadius: "20px",
    padding: "18px 18px 14px",
    boxShadow:
      "0 0 20px rgba(245,166,35,0.08), inset 0 1px 0 rgba(245,166,35,0.15)",
  },
  tipHeader: {
    display: "flex",
    alignItems: "center",
